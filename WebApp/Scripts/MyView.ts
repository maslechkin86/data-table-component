namespace View {
	...
	export class MyView {
		private readonly dataTable1: Component.DataTable;
		private readonly dataTable2: Component.DataTable;
		private readonly tabulatorTable1: View.TabulatorTableView;
		private readonly tabulatorTable2: any;
		...
		private InitializeDataTableFromHtml(container: JQuery): Component.DataTable {
			const columns = [
			...
			const component = new Component.DataTable(container, columns);
			component.RenderFromHtml();
			return component;
		}

		private InitializeDataTable(container: JQuery): Component.DataTable {
			const columns = [
				new Component.TableColumn("id", "Id", true, true, true),
				new Component.TableColumn("timestamp", "Timestamp"),
				new Component.TableColumn("status", "Status"),
			...
			const component = new Component.DataTable(container, columns,
				(data: any) => {
					...
					(<any[]>json.aaData).forEach((v, i) => {
						v.timestamp = moment(v.timestamp).format(this.dateTimeFormat);
					});
		...
		private InitializeTabulator(container: JQuery): any {
			const tabulatorTable = new View.TabulatorTableView(container, {
				height: 400,
				ajaxFiltering: true,
				ajaxSorting: true,
				pagination: "remote",
		...
		private InitializeTabulatorFromHtml(container: string): any {
			const tabulatorTable = new Tabulator(container, {
				pagination: "local",
				paginationSize: 5,
			...
			$(".tabulator-table__download-btn").click(e => {
				e.preventDefault();
